...
public class RegisterScreen extends CustomScreenAdapter {
    ...
    public void show() {
        ...
        TextButton login = new TextButton("Register", Styles.getButtonStyle());
        login.addListener(new ChangeListener() {
            @SneakyThrows
            @Override
            public void changed(ChangeEvent changeEvent, Actor actor) {
                Call<User> registerCall = APIHandler.createService(APICalls.class)
                        .register(new User(username.getText(), password.getText()));
                Response<User> response = registerCall.execute();
